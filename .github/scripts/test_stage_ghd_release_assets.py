...
class StageGhdReleaseAssetsTest(unittest.TestCase):
    def test_stages_expected_assets(self) -> None:
        with fixture() as root:
            ...
            self.assertEqual(
                ...
                [
                    "checksums.txt",
                    "mock-oidc_1.2.3_darwin_amd64",
                    "mock-oidc_1.2.3_darwin_amd64.sbom.json",
                    "mock-oidc_1.2.3_darwin_arm64",
                    "mock-oidc_1.2.3_darwin_arm64.sbom.json",
                    "mock-oidc_1.2.3_linux_amd64",
                    "mock-oidc_1.2.3_linux_amd64.sbom.json",
                    "mock-oidc_1.2.3_linux_arm64",
                    "mock-oidc_1.2.3_linux_arm64.sbom.json",
                ],
            )
            linux_binary = root / "dist/release-assets/mock-oidc_1.2.3_linux_amd64"
            mode = linux_binary.stat().st_mode
            self.assertTrue(mode & stat.S_IXUSR)
            self.assertIn("dist/release-assets/mock-oidc_1.2.3_linux_arm64", stdout)

    def test_fails_on_missing_checksum_entry(self) -> None:
        with fixture(missing_checksum="mock-oidc_1.2.3_linux_arm64") as root:
            result, _, stderr = run_script(root)

            self.assertEqual(result, 1)
            self.assertIn("missing checksum entry", stderr)
            self.assertIn("mock-oidc_1.2.3_linux_arm64", stderr)

    def test_fails_on_checksum_mismatch(self) -> None:
        override = ("mock-oidc_1.2.3_linux_amd64", "0" * 64)
    ...
    def test_fails_on_missing_os_arch_asset(self) -> None:
        with fixture(omit_artifact=("linux", "arm64", "Binary")) as root:
            ...
            self.assertEqual(result, 1)
            self.assertIn("missing expected binary asset mock-oidc_1.2.3_linux_arm64", stderr)
    ...
    *,
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        for goos, goarch in PLATFORMS:
            binary_name = f"mock-oidc_1.2.3_{goos}_{goarch}"
        ...
        if extra_binary:
            extra_name = "mock-oidc_1.2.3_freebsd_amd64"
